...
public:
    ...
    Status get_next(vectorized::Block& output_block, std::vector<vectorized::MutableColumnPtr>& columns, bool* eof) override;
...
private:
    ...
    Status fill_dest_columns(std::vector<MutableColumnPtr>& columns);

private:
    doris::MemPool* tuple_mem_pool;
...
private:
    ...
    Status _handle_simple_json_with_columns(std::vector<MutableColumnPtr>& columns, 
                                        const std::vector<SlotDescriptor*>& slot_descs,
                                        bool* is_empty_row, bool* eof);
    ...
    Status _insert_to_column(rapidjson::Value::ConstValueIterator value, vectorized::IColumn* column_ptr, 
                            SlotDescriptor* slot_desc, const char* value_ptr, int32_t& wbytes);
